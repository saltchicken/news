...
if not df_discoveries.empty:
    ...
    mask = (df_discoveries['timestamp'].dt.date
            >= start_date) & (df_discoveries['timestamp'].dt.date <= end_date)
    ...
    if not df_filtered.empty:
        ...
        display_df = df_filtered[[
            'timestamp', 'ticker', 'sentiment', 'analysis', 'source', 'title',
            'link'
        ]]
                     ...
                     column_config={
                         "link":
                             st.column_config.LinkColumn(
                                 "Article Link", display_text="Open Article")
                     })
    else:
        st.warning("No discoveries found matching your filters.")
else:
    st.warning(
        "No stock discoveries collected yet. Waiting for the JSON to populate..."
    )
